...
class SocketSimulator:
    def __init__(self, timeout=180, tick=2):
        # IP
        self.host = HostSimulator()
    ...
    def send(self, data=''):
        ...
        slice_length = 0
        total_len = len(data)
        while slice_length < total_len:
            self._send(data[slice_length:(slice_length + self.tcp_cwind)], ack=1)
            # update TCP seq
            if (slice_length + self.tcp_cwind) > total_len:
                self.tcp_seq += (total_len - slice_length)
            else:
                self.tcp_seq += self.tcp_cwind
            slice_length += self.tcp_cwind
        return total_len
    ...
    def _recv(self, max_retry, bufsize=1500):
        ...
        while max_retry:
            ...
            if len(self.host.tcp_ip_queue) > 0:
